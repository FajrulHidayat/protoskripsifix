...
class TabelJfu extends React.Component {
  ...
  showModal = async (record) => {
    ...
    axios
      .get(`/jfu/${record.tentang}/${record.id_surat}`, null, headers)
      .then((res) => {
  ...
  handleOk = (e) => {
    ...
    axios
      .put(`/jfu/${this.state.tentang}/${this.state.modalKey}`, body)
      .then((res) => {
        // UbahDom(this.state.redirect,this.props.history)
        console.log(res);
        this.getData();
  ...
  render() {
    ...
    const col =
      this.props.match.path === "/admin/jfu" ? columns[0] : columns[1];
    ...
    return (
      <div>
        ...
        >
          ...
          >
            <Form.Item key={20} name="nomor" label="Nomor">
              <Input />
            </Form.Item>
            {this.state.tentang !== "Pembimbing" ? (
              <Form.Item key={21} name="pelaksana" label="Pelaksana">
                <Input />
              </Form.Item>
            ) : null}
            ...
            {this.state.forms.map((form, index) => {
              return (
                <Form.Item key={index} name={form.name} label={form.label}>
                  <Input disabled={true} />
                </Form.Item>
              );
            })}
